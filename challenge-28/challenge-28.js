...
function CleanNotNumbers() {
  return regexEdit($inputCEP.get()[0].value.replace( /\D/g, '' ));
  }

function regexEdit( input ) {   
   return input.match( /^\d{8}$/g ) ?
      input.replace( /(\d{5})/, '$1-' ) :
...
function replaceCEP( message ) {
  return message.replace( '[CEP]', CleanNotNumbers() );
}
